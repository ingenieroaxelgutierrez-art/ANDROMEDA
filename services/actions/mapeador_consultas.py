    ...
    def _mapear_accion_a_consulta_odoo(self, accion: str, fecha_ini: str, fecha_fin: str, params: dict, consulta) -> dict:
        ...
        mapeo = {
            ...
            'ventas_por_categoria': {
                ...
                'campos': ['product_id', 'price_subtotal', 'product_uom_qty', 'order_id'],
                'limite': 500,
                'orden': 'price_subtotal desc',
            },
            ...
            'movimientos_stock': {
                ...
                'filtro': [('date', '>=', fecha_ini), ('date', '<=', fecha_fin),
                           ('state', '=', 'done')],
                'campos': ['product_id', 'product_qty', 'location_id', 'location_dest_id', 'date', 'picking_id'],
                'limite': 300,
                'orden': 'date desc',
            },
            'productos_bajo_minimo': {
                'modelo': 'product.template',
                'filtro': [('active', '=', True), ('type', '=', 'product')],
                'campos': ['name', 'default_code', 'qty_available', 'reordering_rules_count'],
                'limite': 300,
                'orden': 'qty_available asc',
            },
            'kpi_faltantes': {
                'modelo': 'stock.quant',
                'filtro': [('quantity', '<=', 0)],
            ...
            'facturas_filtradas': {
                'modelo': 'account.move',
                'filtro': filtro_fecha_factura + [('move_type', 'in', ['out_invoice', 'out_refund']), ('state', '=', 'posted')],
                'campos': ['name', 'partner_id', 'amount_total', 'amount_residual', 'invoice_date', 'payment_state', 'move_type'],
                'limite': 200,
                'orden': 'invoice_date desc',
            },
            'consultar_pagos': {
                'modelo': 'account.payment',
                'filtro': [('date', '>=', fecha_ini), ('date', '<=', fecha_fin), ('state', '=', 'posted')],
                'campos': ['name', 'partner_id', 'amount', 'date', 'payment_type', 'journal_id', 'currency_id'],
                'limite': 200,
                'orden': 'date desc',
            },
            ...
            'consultar_crm': {
                'modelo': 'crm.lead',
                'filtro': [('type', '=', 'opportunity'), ('active', '=', True)],
                'campos': ['name', 'partner_id', 'stage_id', 'expected_revenue', 'probability', 'user_id', 'create_date'],
                'limite': 200,
                'orden': 'expected_revenue desc',
            },
            'prediccion_churn': {
            ...
            'analisis_rh': {
                'modelo': 'hr.employee',
                'filtro': [('active', '=', True)],
                'campos': ['name', 'department_id', 'job_id', 'create_date', 'work_email', 'coach_id'],
                'limite': 300,
                'orden': 'department_id asc',
            },
            'headcount': {
                'modelo': 'hr.employee',
                'filtro': [('active', '=', True)],
                'campos': ['name', 'department_id', 'job_id', 'create_date', 'resource_calendar_id'],
                'limite': 500,
                'orden': 'department_id asc',
            },
            'rotacion_personal': {
            ...
            'consultar_usuarios': {
                'modelo': 'res.users',
                'filtro': [('active', '=', True)],
                'campos': ['login', 'name', 'groups_id', 'login_date', 'share', 'create_date'],
                'limite': 100,
                'orden': 'name asc',
            },
            'actividad_usuarios': {
                ...
                'filtro': [('date', '>=', fecha_ini), ('date', '<=', fecha_fin),
                           ('author_id', '!=', False)],
                'campos': ['model', 'res_id', 'date', 'author_id', 'message_type'],
                'limite': 300,
                'orden': 'date desc',
            },
            'consultar_proyectos': {
                'modelo': 'project.project',
                'filtro': [('active', '=', True)],
                'campos': ['name', 'user_id', 'partner_id', 'date_start', 'date', 'stage_id', 'task_count'],
                'limite': 100,
                'orden': 'name asc',
            },
            'tareas': {
                'modelo': 'project.task',
                'filtro': [('active', '=', True)],
                'campos': ['name', 'project_id', 'user_ids', 'stage_id', 'date_deadline', 'priority', 'kanban_state'],
                'limite': 200,
                'orden': 'date_deadline asc',
            },

            # ── Matemáticas — data base para cálculos ──
            'calculo_roi': {
                'modelo': 'sale.order',
                'filtro': filtro_fecha_venta,
                'campos': ['date_order', 'amount_total', 'amount_untaxed', 'user_id'],
                'limite': 500,
                'orden': 'date_order asc',
            },
            'calculo_margenes': {
                ...
                'filtro': [('order_id.date_order', '>=', fecha_ini), ('order_id.date_order', '<=', fecha_fin),
                           ('order_id.state', 'in', ['sale', 'done'])],
                'campos': ['product_id', 'price_unit', 'price_subtotal', 'product_uom_qty', 'discount'],
                'limite': 500,
                'orden': 'price_subtotal desc',
            },
            'calculo_rentabilidad': {
                'modelo': 'account.move',
                'filtro': filtro_fecha_factura + [('move_type', 'in', ['out_invoice', 'out_refund']), ('state', '=', 'posted')],
                'campos': ['partner_id', 'amount_total', 'amount_untaxed', 'amount_tax', 'invoice_date', 'move_type'],
                'limite': 500,
                'orden': 'invoice_date desc',
            },
            'calculo_break_even': {
                ...
                'filtro': [('order_id.date_order', '>=', fecha_ini), ('order_id.date_order', '<=', fecha_fin),
                           ('order_id.state', 'in', ['sale', 'done'])],
                'campos': ['product_id', 'price_unit', 'price_subtotal', 'product_uom_qty'],
                'limite': 500,
                'orden': 'price_subtotal desc',
            },
            'calculo_punto_equilibrio': {
                'modelo': 'sale.order.line',
                'filtro': [('order_id.date_order', '>=', fecha_ini), ('order_id.date_order', '<=', fecha_fin),
                           ('order_id.state', 'in', ['sale', 'done'])],
                'campos': ['product_id', 'price_unit', 'price_subtotal', 'product_uom_qty'],
            ...
            'calculo_tir': {
                'modelo': 'account.move',
                'filtro': [('state', '=', 'posted'), ('move_type', 'in', ['out_invoice', 'in_invoice'])],
                'campos': ['move_type', 'amount_total', 'invoice_date'],
                'limite': 500,
                'orden': 'invoice_date asc',
            },
            'calculo_vpn': {
            ...
            'calculo_amortizacion': {
                'modelo': 'account.move.line',
                'filtro': [('account_id.account_type', 'in', ['liability_payable', 'asset_receivable']),
                           ('parent_state', '=', 'posted')],
                'campos': ['account_id', 'debit', 'credit', 'balance', 'date'],
                'limite': 300,
                'orden': 'date asc',
            },
            'analisis_sensibilidad': {
                'modelo': 'sale.order',
                'filtro': filtro_fecha_venta,
                'campos': ['date_order', 'amount_total', 'amount_untaxed'],
                'limite': 500,
                'orden': 'date_order desc',
            },
            'calculo_payback': {
                'modelo': 'account.move',
                'filtro': [('state', '=', 'posted'), ('move_type', '=', 'out_invoice')],
                'campos': ['amount_total', 'invoice_date', 'partner_id', 'payment_state'],
                'limite': 500,
                'orden': 'invoice_date asc',
            },
            'calculo_wacc': {
                'modelo': 'account.move.line',
                'filtro': [('account_id.account_type', 'in', ['equity', 'liability_payable']),
                           ('parent_state', '=', 'posted')],
                'campos': ['account_id', 'debit', 'credit', 'balance'],
                'limite': 200,
            },
            'depreciacion': {
                ...
                'filtro': [('state', 'in', ['open', 'draft'])],
                'campos': ['name', 'value', 'value_residual', 'depreciation_line_ids', 'method', 'method_number'],
                'limite': 100,
                'orden': 'value desc',
            },
            'calculo_elasticidad': {
                'modelo': 'sale.order.line',
                'filtro': [('order_id.date_order', '>=', fecha_ini), ('order_id.date_order', '<=', fecha_fin)],
                'campos': ['product_id', 'price_unit', 'product_uom_qty', 'discount'],
                'limite': 500,
                'orden': 'product_id asc',
            },
            'analisis_apalancamiento': {
                'modelo': 'account.move.line',
                'filtro': [('account_id.account_type', 'in', ['asset_current', 'liability_current', 'equity']),
                           ('parent_state', '=', 'posted')],
                'campos': ['account_id', 'debit', 'credit', 'balance'],
                'limite': 500,
            },
            'calculo_dupont': {
                'modelo': 'account.move',
                'filtro': [('state', '=', 'posted'), ('move_type', 'in', ['out_invoice', 'in_invoice'])],
                'campos': ['move_type', 'amount_total', 'amount_untaxed', 'invoice_date'],
                'limite': 500,
                'orden': 'invoice_date desc',
            },
            'calculo_capital_requerido': {
                'modelo': 'account.move.line',
                'filtro': [('account_id.account_type', 'in', ['asset_current', 'liability_current']),
                           ('parent_state', '=', 'posted')],
                'campos': ['account_id', 'debit', 'credit', 'balance'],
                'limite': 500,
            },
            ...
            'distribucion_datos': {
                'modelo': 'sale.order',
                'filtro': filtro_fecha_venta,
                'campos': ['date_order', 'amount_total', 'user_id'],
                'limite': 500,
                'orden': 'amount_total desc',
            },
            'outliers_datos': {
            ...
            'kpis_empresariales': {
                'modelo': 'sale.order',
                'filtro': filtro_fecha_venta,
                'campos': ['date_order', 'amount_total', 'partner_id', 'user_id', 'state'],
                'limite': 500,
                'orden': 'date_order desc',
            },
            'analisis_rfm': {
            ...
            'benchmarking': {
                'modelo': 'sale.order',
                'filtro': filtro_fecha_venta,
                'campos': ['date_order', 'amount_total', 'user_id', 'team_id'],
                'limite': 500,
                'orden': 'user_id asc',
            },
            ...
            'analisis_canasta': {
                ...
                'filtro': [('order_id.date_order', '>=', fecha_ini), ('order_id.date_order', '<=', fecha_fin),
                           ('order_id.state', 'in', ['sale', 'done'])],
                'campos': ['product_id', 'order_id', 'price_subtotal', 'product_uom_qty'],
                'limite': 500,
                'orden': 'order_id asc',
            },
            ...
            'ranking_multidimensional': {
                'modelo': 'sale.order',
                'filtro': filtro_fecha_venta,
                'campos': ['partner_id', 'user_id', 'amount_total', 'date_order'],
                'limite': 500,
                'orden': 'amount_total desc',
            },
            'kpis_personalizados': {
            ...
            'forecast_financiero': {
                'modelo': 'account.move',
                'filtro': [('state', '=', 'posted'), ('move_type', 'in', ['out_invoice', 'in_invoice'])],
                'campos': ['move_type', 'amount_total', 'invoice_date', 'invoice_date_due', 'payment_state'],
                'limite': 500,
                'orden': 'invoice_date asc',
            },
            'forecast_inventario': {
            ...
            'prediccion_demanda': {
                ...
                'filtro': [('order_id.date_order', '>=', fecha_ini), ('order_id.date_order', '<=', fecha_fin),
                           ('order_id.state', 'in', ['sale', 'done'])],
                'campos': ['product_id', 'product_uom_qty', 'price_subtotal', 'order_id'],
                'limite': 500,
                'orden': 'product_id asc',
            },
            'prediccion_demanda_producto': {
                'modelo': 'sale.order.line',
            ...
            'prediccion_rotacion_personal': {
                'modelo': 'hr.employee',
                'filtro': [],
                'campos': ['name', 'department_id', 'active', 'create_date', 'departure_date'],
                'limite': 500,
                'orden': 'create_date desc',
            },
