# 🗺️ UML ANDRÓMEDA — El Mapa del Tesoro

> **Fecha:** Marzo 2026  
> **Versión:** 7.5  
> **Propósito:** Documentar la arquitectura completa para el "yo del futuro"  
> **Tip:** Abre este archivo en cualquier visor Mermaid (VS Code, GitHub, Notion, draw.io)  
> **Última actualización:** 25/25 fixes implementados (ver MEJORAS.md)  
...
| **Presentación** | `views/interfaz_v5.py`, `views/generador_reportes.py`, `services/formatters/formateador_respuestas.py`, `services/agents/ejecutores.py` | `InterfazAndromeda`, `GeneradorReportes`, `FormateadorRespuestas`, `EjecutoresAgente` | UI Gradio (validación entrada + rate limiting), formateo delegado (41 métodos), ejecutores delegados (12), pipeline anti-alucinación, reportes Excel/PDF/HTML |
...
| **NLP** | `services/nlp/*.py` (5 archivos) | `MotorNLPAvanzado`, `MotorNLP`, `CerebroNLP`, `MotorEmbeddings`, `MotorEmpatico` | 90+ intenciones, entidades, embeddings semánticos (`paraphrase-multilingual-MiniLM-L12-v2`, 384 dims), cache SHA-256 auto-invalidante, empatía |
...
| **Agentes** | `services/agents/multi_agente.py`, `ejecutores.py` | `GestorMultiAgente`, 12 `Agent*` + `AgentValidadorFinal`, `PasoAgente`, `ResultadoCadena` | Routing inteligente + cadena multi-agente con ejecución real + ejecutores dedicados + enriquecimiento + validación triple |
| **Análisis** | `services/analysis/*.py` (7 archivos) | `Analizador360`, `AnalizadorAnomalias`, `AnalizadorAvanzado`, `DetectorContexto`, `KPIs*` | Análisis 360°, anomalías financieras, KPIs empresariales (60+), dashboard ejecutivo |
...
| **Memoria** | `services/memory/*.py` (3 archivos) | `MemoriaVectorial`, `MemoriaJerarquica`, `GrafoConocimiento`, `ExtractorEntidades` | ChromaDB con EF explícita (lazy), 6 colecciones, purga selectiva, grafo NetworkX (14 nodos / 9 relaciones), poda proactiva, auto-save predecible, sanitización metadatos |
...
| **Datos** | `models/*.py` | `ConectorOdoo`, `ModeloOdoo` | Conexión Odoo RPC, cache, 40+ modelos definidos |
| **Utilidades** | `utils/*.py` (8 archivos) | `ValidadorDatos`, `ValidadorRespuestas`, `ConsultasEspecializadas`, `MonitorSistema`, `NormalizadorPrompt`, `LoggerAvanzado` | Validación, queries especializadas, monitoreo, logging SQLite, 90+ intenciones extendidas |
...
|--------|----------------|-------------|
| **MVC++** | `OdooBotPro` → `ConectorOdoo` → `OdooAIProV5` | Modelo (Odoo) → Controlador (Bot/Cerebro) → Vista (Gradio) |
| **Strategy** | `GestorMultiAgente` + `Agent*` | Selecciona el agente especializado según la acción detectada |
| **Pipeline / Chain** | `ejecutar_cadena_completa()` → por cada agente: executor → `enriquecer_respuesta()` → `post_ejecucion()` → `ValidadorFinal` | Múltiples agentes EJECUTAN secuencialmente, cada uno añade análisis real sobre datos |
| **Factory** | `ConectorOdoo` | Crea instancias de modelos Odoo dinámicamente por nombre |
| **Singleton** | `ValidadorDatos`, `ManejadorErrores` | Una sola instancia global con `obtener_validador()` |
...
| **Delegation** | `OdooAIProV5` → `FormateadorRespuestas`, `EjecutoresAgente` | La interfaz delega formateo (41 métodos) y ejecución (12 ejecutores) a clases especializadas vía composición |
